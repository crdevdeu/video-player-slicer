...
/*
  ...
  is the full length video . Each one of the components handlers executes an action 
  creator to modify the application state
*/
          ...
          <li key={index} className={vm.props.tagsearchvalue !== '' && item.tagname !== vm.props.tagsearchvalue ? "hide" : "list-group-item"}>
            ...
            {!(vm.props.videoisplaying && vm.props.indexofcurrentclip === index) ?
              <div>
                ...
                {vm.allowedit ? 
                  <button 
                    className="btn btn-primary button-interphase" 
                    onClick={vm.editClip.bind(vm,index)}
                  >
                    Edit
                  </button> 
                :null}
                <button 
                  className="btn btn-danger button-interphase" 
                  onClick={vm.deleteClip.bind(vm,index)}
                >
                  Delete
                </button>
                {!item.persisted ?
                  <button 
                    className="btn btn-warning button-interphase"
                    onClick={vm.persistClip.bind(vm,index)}
                  >
                    Save
                  </button>
                : null}
